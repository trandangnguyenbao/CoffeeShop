        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            if (isset($_POST['user_id']) && !empty($_POST['user_id']) && isset($_POST['old_password']) && !empty($_POST['old_password']) && isset($_POST['new_password']) && !empty($_POST['new_password'])
                ...
                if ($userResult->num_rows > 0) {
                    $result = mysqli_query($conn, "UPDATE `user` SET `password` = MD5('" . $_POST['new_password'] . "') WHERE (`id` = " . $_POST['user_id'] . " AND `password` = '" . md5($_POST['old_password']) . "')");
                    if (!$result) {
                ...
                <?php } else { ?>
                    <div id="edit-notify" class="box-content">
                        ...
                        <a href="./quanli.php">Quay lại tài khoản</a>
                    </div>
                <?php } ?>
            <?php } else { ?>
                <div id="edit-notify" class="box-content">
